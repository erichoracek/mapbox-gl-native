...
void mbx_glDeleteTextures(GLsizei n,
    ...
    for(int i = 0; i < n; ++i) {
        if(binding_to_size_map.find(textures[i]) != binding_to_size_map.end()) {
            ...
            //mbgl::Log::Record(mbgl::EventSeverity::Info, mbgl::Event::OpenGL, "GL deleteTexture: %d freeing %d bytes current total %d", textures[i], binding_to_size_map[textures[i]], current_used_bytes);
...
void mbx_glTexImage2D(GLenum target,
    ...
    if (internalformat == GL_RGBA &&
        ...
        if (binding_to_size_map.find(current_bound_texture) != binding_to_size_map.end()) {
            ...
            //mbgl::Log::Record(mbgl::EventSeverity::Info, mbgl::Event::OpenGL, "GL glTexImage2D: %d freeing %d bytes current total %d", current_bound_texture, binding_to_size_map[current_bound_texture], current_used_bytes);
            
        }
        binding_to_size_map[current_bound_texture] = width * height * 4;
        current_used_bytes += binding_to_size_map[current_bound_texture];
        std::cout << "GL glTexImage2D: " << current_bound_texture << " freeing " << binding_to_size_map[current_bound_texture] << " bytes current total " << current_used_bytes << "\n";
        //mbgl::Log::Record(mbgl::EventSeverity::Info, mbgl::Event::OpenGL, "GL glTexImage2D: %d using %d bytes current total %d", current_bound_texture, binding_to_size_map[current_bound_texture], current_used_bytes);
